...
/*void main() {
	float noise = snoise(vec3(textureUV, 0.0f) * 5.0f);
	float forestDensity = noise;
	ivec2 treeSeed = ivec2(textureUV * treeDensity);
	ivec2 grassSeed = ivec2(textureUV * grassDensity);
	float val = (rand2D(treeSeed) * 0.5f + 0.5f) * ((worldHeight - 0.3f) + 0.3f);
	float treePresentVal = rand2D(treeSeed + ivec2(1, 1));
	vec2 offset = (vec2(rand2D(treeSeed + ivec2(1, 0)), rand2D(treeSeed + ivec2(0, 1))) * 2.0f - 1.0f) * 0.5f;
	bool isGrass = val > u_height && worldHeight > 0.4f && treePresentVal < forestDensity; 

	// In local space
	vec2 localPos = fract(textureUV * treeDensity);
	...
	vec3 objColor;
	float radius = 0;
	if(u_height < val * 0.5f) {
		radius = 0.3f;
		objColor = vec3(0.5f, 0.3f, 0.0f);
	} else {
		radius = 1.0f - 1.0f * (u_height - val * 0.5f) / (val * 0.5f);
		objColor = vec3(0.0f, 1.0f, 0.0f);
	}

	if(u_height == 0) {
		if(worldHeight > 0.31f) {
			objColor = vec3(0.9f, 0.8f, 0.2f);
			if(worldHeight > 0.39f) {
				objColor = vec3(0.0f, 0.5f, 0.0f);
			}
		} else {
	...
	if (u_height > 0 && (!isGrass || dist > radius)) {
		discard;
	}
	...
	//objColor *= lambert;
	objColor *= u_height * 0.5f + 0.5f;

	// Add specular reflections for water (worldHeight <= 0.3f)
	if(worldHeight <= 0.31f) {
		vec3 viewDir = normalize(u_cameraPosition - worldPos);
		...
		vec3 reflectDir = reflect(lightDir, tnormal);
		float spec = pow(max(dot(viewDir, reflectDir), 0.0f), 32.0f);
		objColor += vec3(0.5f, 0.5f, 0.5f) * spec;
	}
